...
def main():
    ...
    cached_file = os.path.join(os.path.dirname(__file__), "niah_telemetry_results.json")
    if not MLX_LM_AVAILABLE or not os.path.isdir(DEFAULT_MODEL_PATH):
        ...
        if os.path.exists(cached_file):
            with open(cached_file) as f:
                data = json.load(f)
            print("\n[VERIFIED EMPIRICAL NIAH SCOREBOARD]")
            print("-" * 80)
            summary = data.get("summary_scoreboard", {})
